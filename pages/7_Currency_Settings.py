...
# ==============================================================================
# 1. SAFE DATA FETCHING WITH GRACEFUL FALLBACK
# ==============================================================================
...
try:
    # Attempt to fetch from the correct table
    res = supabase.table('currency_rates').select('*').order('currency_code').execute()
    
    if res.data and len(res.data) > 0:
        df_rates = pd.DataFrame(res.data)
        st.success("✅ Successfully loaded currency data from database!")
    else:
        st.warning("⚠️ Query returned 0 rows. This usually means Row Level Security (RLS) is blocking the app from reading the `currency_rates` table. Please run the RLS fix in Supabase SQL Editor.")
...
# ==============================================================================
# 2. UI: VIEW CURRENT RATES
# ==============================================================================
st.markdown("### 📊 Current Exchange Rates")

if df_rates.empty:
    st.info("No currency data could be loaded. Please check the System Status above.")
else:
    # Safely identify the exact column names
    ...
    if rate_col in display_df.columns:
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            code_col: st.column_config.TextColumn("Currency", width="small"),
...
with tab_edit:
    if df_rates.empty or code_col not in df_rates.columns:
        st.warning("No currencies available to edit. Ensure data is loaded above.")
    else:
